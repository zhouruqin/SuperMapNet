...
class VectorizedAV2LocalMap(object):
    ...
    def gen_vectorized_samples(self, location, map_elements, lidar2global_translation, lidar2global_rotation):
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                ...
                ped_instance_list = self.line_geoms_to_instances(ped_geom, False)
        ...
        # filter out -1
        div=line_geom[0][1]
        div = remove_boundary_dividers(div, poly_bound_list)
        #div = remove_ped_dividers(div, ped_instance_list)
        map_divider_geom=[]
        map_divider_geom.append(('divider', div ))

        vectors = []
        for divider, length in self._one_type_line_geom_to_instances(self.line_geoms_to_instances(map_divider_geom, False)):
            vectors.append((divider, length, self.CLASS2LABEL.get('divider', -1)))
        for instance, length in self._one_type_line_geom_to_instances(ped_instance_list):
            vectors.append((instance, length, self.CLASS2LABEL.get('ped_crossing', -1)))
        for bound, length in self._one_type_line_geom_to_instances(poly_bound_list):
    ...
    def bound_poly_geoms_to_instances(self, polygon_geom):
        ...
        return  results#self._one_type_line_geom_to_instances(results)

    def line_geoms_to_instances(self, line_geom, is_True):
        lines = line_geom[0][1]
