...
class VectorizeMap(object):
                ...
                if not geom.is_empty:
                    if geom.geom_type == 'MultiLineString':
                        for single_line in geom.geoms:
                            vectors[label].append(self.sample_pts_from_line(single_line))
                    elif geom.geom_type == 'LineString':
                        vectors[label].append(self.sample_pts_from_line(geom))
    ...
    def sample_pts_from_line(self, line):
        ...
        if not self.padding or self.fixed_num > 0:
            ...
            #print('0', sampled_points.shape)
            return sampled_points[:, :self.coords_dim]#, num_valid
    ...
    def __call__(self, input_dict):
        ...
        #sd_map_data = input_dict.get('sd_vectors', None)
        input_dict['vectors'] = self.get_vectorized_lines(map_geoms)
        #input_dict['sd_vectors'] = sd_map_data
        return input_dict
