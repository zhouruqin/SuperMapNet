...
from tools.evaluation.eval import format_print
...
class BeMapNetEvaluator(BaseExecutor):
    ...
    def eval(self, ckpt_name=None):
        ...
        for step in tqdm(range(len(self.test_dataloader))):
            batch_data = next(test_iter)
            with torch.no_grad():
                ap_matrix, ap_count_matrix = exp.test_step(batch_data, step, ap_matrix, ap_count_matrix)
            self._invoke_callback("after_step", step, {})
            #exit()
        '''end_time = time.time()
        fps = len(self.test_dataloader)/(end_time-start_time)
        print(f"Inference Speed: {fps:.2f} FPS")'''
        synchronize()

        if get_rank() == 0:
            self.logger.info("Done with inference, start evaluation later!") 
            ap = (ap_matrix / ap_count_matrix).cpu().data.numpy()
            format_print(ap)
            self.logger.info(ap)
